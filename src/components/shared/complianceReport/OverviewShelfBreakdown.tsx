...
export function OverviewShelfBreakdown({
  ...
  const shelfIssueMap = buildShelfIssueMap(shelfCompliance, issuesToReview);
    ...
    <div className="space-y-2">
      {shelfCompliance.map((shelf) => {
        ...
        const issueByName = new Map(
          shelfIssues.map(
            (entry) => [entry.name.toLowerCase(), entry.status] as const,
          ),
        );
        const misplacedCount = shelfItems.filter(
          (item) =>
            issueByName.get(item.productName.toLowerCase()) === "misplaced",
        ).length;
        const missingCount = shelfIssues.filter(
          (item) => item.status === "missing",
        ).length;
        const extraCount = shelfIssues.filter(
        ...
        return (
          ...
          >
              ...
              <div className="bg-muted/60 h-2 flex-1 overflow-hidden rounded-full">
                <div
                  className={cn(
                    ...
                    shelf.compliance >= 80
                      ? "from-chart-2/85 to-chart-2 bg-linear-to-r"
                      : shelf.compliance > 0
                        ? "from-action-warning/85 to-action-warning bg-linear-to-r"
                        : "bg-destructive/70",
                  )}
            ...
            {nonCompliantItems.length > 0 ? (
              <div className="mb-0.5 flex flex-wrap gap-1">
                {visibleItems.map((item) => {
                  const status =
                    issueByName.get(item.productName.toLowerCase()) ??
                    "matched";
                  return (
                    <span
                      key={item.id}
            ...
            <p className="text-muted-foreground text-[10px]">
              {misplacedCount} misplaced · {missingCount} missing · {extraCount}{" "}
              extra
            </p>
            {shelfItems.length > 0 && (
