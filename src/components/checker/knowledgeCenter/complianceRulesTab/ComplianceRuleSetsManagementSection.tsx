...
function actionCellHtml(isDefault: boolean): string {
  ...
  const deleteIcon = renderToStaticMarkup(
...
export function ComplianceRuleSetsManagementSection() {
  ...
  const filteredSets = useMemo(() => {
    ...
    return complianceRuleSets.filter(
  ...
  const ruleSetDetailMap = useMemo(() => {
    const entries = complianceRuleSets.map(
      (ruleSet, idx) => [ruleSet.id, ruleSetDetailsQueries[idx]?.data] as const,
    );
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      payload: Parameters<typeof updateComplianceRuleSet>[1];
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({
        ...
        description: "The rule set was removed successfully.",
      });
    } catch (err) {
      toast({
        title: "Delete failed",
