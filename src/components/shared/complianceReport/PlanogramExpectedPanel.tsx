...
import {
  ...
  ChipBagSVG,
  LargeBottleSVG,
} from "@/components/planogram/ProductSvgs";
import type {
  ImageComparisonData,
  PlanogramSlot,
  PlanogramSlotStatus,
} from "@/lib/analysis/imageComparisonTypes";
...
function PlanogramSlotCard({
  ...
  const borderByStatus: Record<PlanogramSlotStatus, string> = isPreview
    ...
    : {
        matched: slot.highDemand
          ? "border-2 border-amber-500"
          : "border-2 border-chart-2",
        missing: "border-2 border-destructive",
        misplaced: slot.highDemand
          ? "border-2 border-amber-500"
          : "border-2 border-action-warning",
      };
  ...
  return (
    <div
      className={cn(
        "bg-card/80 relative flex min-h-[62px] flex-col items-center rounded-md px-1 py-1",
        borderByStatus[slot.status],
      )}
    >
      {!isPreview && slot.severity === "HIGH" && (
        <span className="bg-destructive/90 absolute top-1.5 left-1.5 rounded px-1.5 py-0.5 text-[10px] font-bold text-white uppercase">
          HIGH
        </span>
      )}
      {!isPreview &&
        ...
        slot.severity !== "HIGH" && (
          <Star
            className="absolute top-1.5 left-1.5 size-3.5 fill-amber-500 text-amber-500"
            aria-hidden
          />
        )}
      {!isPreview ? (
        ...
        <span className="text-muted-foreground absolute top-1 right-1 rounded px-1 py-0 text-[9px] font-medium">
      ...
      <p className="text-foreground mt-0.5 w-full truncate text-center text-[10px] font-medium">
        {slot.shortName}
      </p>
      <p
        className={cn(
...
export function PlanogramExpectedPanel({
  ...
  return (
    <section
      className={cn(
        "border-border bg-card/60 flex min-h-0 flex-col overflow-hidden rounded-xl border",
        className,
      )}
    >
      ...
      {showLegend && (
        <div className="border-border text-muted-foreground flex shrink-0 flex-wrap gap-3 border-t px-3 py-1.5 text-[10px]">
          {variant === "comparison" ? (
            <>
