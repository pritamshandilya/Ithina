...
export async function submitFixtureAnalysis(
  ...
  const response = await axiosClient.post<AnalysisJobResponse>(
    `/fixtures/${fixtureId}/analyze`,
    formData,
  );
  return {
    job: response.data,
    mercureHubUrl: parseMercureLinkHeader(
      response.headers["link"] as string | undefined,
    ),
    mercureAuthorization:
      (response.headers["mercure-authorization"] as
        | string
        | undefined
        | null) ?? null,
  };
...
export function fetchFixtureAnalysisJobs(
  ...
  return axiosClient
    .get<AnalysisJobResponse[]>(`/fixtures/${fixtureId}/analysis`)
    .then((res) => res.data);
}

export function fetchAnalysisJob(jobId: string): Promise<AnalysisJobResponse> {
  return axiosClient
    .get<AnalysisJobResponse>(`/analysis/${jobId}`)
    .then((res) => res.data);
}
