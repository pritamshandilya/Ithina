...
function parseDimensions(value: unknown, path: string): PlanogramDimensions {
...
function parseProduct(value: unknown, path: string): PlanogramProduct {
  ...
  return {
    ...
    depth_count: parseRequiredNumber(
      value.depth_count,
      `${path}.depth_count`,
      1,
    ),
    ...(parseOptionalNumber(value.velocity, `${path}.velocity`, 0) != null
      ...
      : {}),
    expiry_sensitive:
      typeof value.expiry_sensitive === "boolean"
        ? value.expiry_sensitive
        : false,
  };
}
