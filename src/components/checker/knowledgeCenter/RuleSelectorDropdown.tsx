...
import type { RuleOption } from "./referenceDocumentsTab.types";
...
export function RuleSelectorDropdown({
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = () => {
      ...
      const panelWidth =
        panelRef.current?.offsetWidth ?? Math.max(280, triggerRect.width);
  ...
  return (
    <div className="relative">
      <button
        ...
        className={
          triggerClassName ??
          "border-border bg-card hover:bg-muted/50 focus:ring-ring flex w-full min-w-[200px] items-center justify-between gap-2 rounded-md border px-3 py-2 text-left text-sm focus:ring-2 focus:outline-none"
        }
      ...
      {isOpen && !usePortal && (
        <div
          ...
          className={
            inlinePanel
              ? "border-border bg-popover mt-1 w-full min-w-0 rounded-md border shadow-sm"
              : `border-border bg-popover absolute top-full z-50 mt-1 max-w-[min(90vw,420px)] min-w-[280px] rounded-md border shadow-lg ${panelAlign === "right" ? "right-0" : "left-0"}`
          }
        >
            ...
            <div className="mt-2 flex gap-1">
              ...
              >
                {filteredRules.every((rule) =>
                  selectedIds.includes(rule.ruleId),
                )
                  ? "Deselect all"
                  : "Select all"}
              </Button>
      ...
      {isOpen &&
        ...
        createPortal(
          <div
            ref={panelRef}
            className="border-border bg-popover fixed z-100 max-w-[min(90vw,420px)] rounded-md border shadow-lg"
            style={{
              top: panelStyle.top,
