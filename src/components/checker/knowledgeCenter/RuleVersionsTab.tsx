...
import { Archive, Copy, Pencil, Play, Search } from "lucide-react";
...
const VERSION_STATUS_OPTIONS: RuleVersionStatus[] = [
  "Draft",
  "Active",
  "Archived",
  "Retired",
...
export function RuleVersionsTab() {
  ...
  const [activateConfirmRuleId, setActivateConfirmRuleId] = useState<
    string | null
  >(null);
  const [retireConfirmRuleId, setRetireConfirmRuleId] = useState<string | null>(
    null,
  );
  const [cloneConfirmRuleId, setCloneConfirmRuleId] = useState<string | null>(
    null,
  );
  ...
  const handleOpenMenu = useCallback(
    (row: VersionDisplayRow, anchor: { x: number; y: number }) => {
      const menuWidth = 192;
      const padding = 8;
  ...
  const confirmRetire = useCallback(() => {
    ...
    retireRule.mutate(retireConfirmRuleId, {
      onSuccess: () => {
        toast({
          title: "Rule retired",
          description: "The rule has been retired.",
        });
      ...
      onError: (err) => {
        toast({
          title: "Retire failed",
          description:
            err instanceof Error ? err.message : "Could not retire rule.",
          variant: "destructive",
        });
        setRetireConfirmRuleId(null);
      },
    ...
    cloneRule.mutate(
      ...
      },
      {
        onSuccess: () => {
          toast({
            title: "Rule cloned",
            description: "A new draft rule has been created.",
          });
        ...
        onError: (err) => {
          toast({
            title: "Clone failed",
            description:
              err instanceof Error ? err.message : "Could not clone rule.",
            variant: "destructive",
          });
          setCloneConfirmRuleId(null);
        },
      ...
      },
      {
        ...
        formatter: (c) =>
          versionStatusBadgeHtml(
            (c as { getData: () => VersionDisplayRow }).getData().version
              .status,
          ),
      },
      {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0">
        <h2 className="text-foreground text-lg font-semibold">Rule Versions</h2>
        <p className="text-muted-foreground text-sm">
          View version history, compare changes, and manage rules (edit,
          activate, retire, clone)
        </p>
      </div>
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="min-w-[180px]">
            ...
            <Select
              ...
              onChange={(e) =>
                setVersionFilter(
                  (e.target.value || "") as RuleVersionStatus | "",
                )
              }
            >
      ...
      <div className="mt-4 flex min-h-0 flex-1 flex-col overflow-hidden">
        {isLoading ? (
          ...
          <div className="border-border bg-card text-destructive rounded-lg border p-6">
            Failed to load versions. Please try again.
          </div>
        ) : !filteredRows.length ? (
          <div className="border-border bg-card/50 flex min-h-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed p-12 text-center">
      ...
      {actionsMenu && (
        <div
          ref={actionsMenuRef}
          className="border-border bg-popover fixed z-50 min-w-40 overflow-hidden rounded-md border p-1 shadow-md"
          style={{
            left: actionsMenu.anchor.x,
            top: actionsMenu.anchor.y,
          }}
        >
          {actionsMenu.row.rule &&
            actionsMenu.row.rule.status !== "Retired" && (
              <button
                type="button"
                className="hover:bg-accent hover:text-accent-foreground flex w-full cursor-pointer items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none [&_svg]:size-4 [&_svg]:shrink-0"
                onClick={() => {
