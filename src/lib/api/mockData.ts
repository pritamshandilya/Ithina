  ...
  return shelvesWithFixtureIds.map((shelf, index) => {
    ...
    return {
      ...
      lastAuditDate: audited
        ? randomPastDate(((index + variantSeed) % 6) + 1)
        : undefined,
  ...
  shelves.forEach((shelf) => {
    if (shelf.status !== "never-audited" && shelf.lastAuditDate) {
      const mode: Audit["mode"] =
        Math.random() > 0.5 ? "planogram-based" : "adhoc";
      const audit: Audit = {
        id: `audit-${shelf.id}`,
        shelfId: shelf.id,
      ...
      if (shelf.status === "returned") {
        ...
        audit.rejectedAt = new Date(
          shelf.lastAuditDate.getTime() + 2 * 60 * 60 * 1000,
        );
      ...
      if (shelf.status === "approved") {
        audit.approvedAt = new Date(
          shelf.lastAuditDate.getTime() + 1 * 60 * 60 * 1000,
        );
...
export function generateMockQuickStats(storeId?: string): QuickStats {
  ...
  const pendingCount = shelves.filter(
    (shelf) => shelf.status === "pending",
  ).length;
  const returnedCount = shelves.filter(
    (shelf) => shelf.status === "returned",
  ).length;
 ...
 */
export function generateMakerDashboardStats(
  storeId?: string,
): MakerDashboardStats {
  const shelves = generateMockShelves(storeId);
  const now = new Date();
...
export function generateMockAdhocAnalyses(storeId?: string): AdhocAnalysis[] {
  ...
  const statuses: AdhocAnalysis["status"][] = [
    "completed",
    "completed",
    "processing",
    "completed",
  ...
  const fixtureContexts = [
    {
      fixtureId: "fixture-a3-1",
      fixtureName: "Aisle 3 Beverages Fixture",
      shelfId: "shelf-aisle3-bev-01",
      shelfName: "Aisle 3 Beverages",
      zone: "Grocery",
    ...
    {
      fixtureId: "fixture-a2-2",
      fixtureName: "Snacks Fixture",
      shelfId: "shelf-snacks-02",
      shelfName: "Snacks Bay",
      zone: "Grocery",
    ...
    {
      fixtureId: "fixture-a4-1",
      fixtureName: "Dairy Fixture",
      shelfId: "shelf-dairy-03",
      shelfName: "Dairy Section",
      zone: "Dairy",
      section: "Milk & Yogurt",
      fixtureType: "cooler_shelving",
      dimensions: "800×1800 mm",
    },
    {
      fixtureId: "fixture-a5-2",
      fixtureName: "Frozen Fixture",
      shelfId: "shelf-frozen-04",
      shelfName: "Frozen Foods",
      zone: "Frozen",
      section: "Ice Cream & Desserts",
      fixtureType: "freezer_shelving",
      dimensions: "1000×1900 mm",
    },
    {
      fixtureId: "fixture-a6-1",
      fixtureName: "Bakery End Cap Fixture",
      shelfId: "shelf-bakery-05",
      shelfName: "Bakery End Cap",
      zone: "Bakery",
      section: "Bread & Pastries",
      fixtureType: "gondola",
      dimensions: "900×1600 mm",
    },
  ];
  return names.map((name, i) => {
    ...
    return {
      ...
      status: statuses[i],
      complianceScore:
        statuses[i] === "completed" ? randomScore(72, 98) : undefined,
      errorMessage:
        statuses[i] === "failed"
 ...
 */
export function generateMockComplianceOverview(
  storeId?: string,
): ComplianceOverview {
  // Base numbers, adjust based on store if needed
  const basePending =
    storeId === "store-9012" ? 12 : storeId === "store-5678" ? 5 : 8;
...
export function generateMockPendingAudits(_storeId: string): CheckerAudit[] {
  ...
  pendingShelves.forEach((shelf, index) => {
    ...
    const mode: "planogram-based" | "adhoc" =
      index % 2 === 0 ? "planogram-based" : "adhoc";
  ...
  return pendingAudits.sort(
    (a, b) => (a.complianceScore || 0) - (b.complianceScore || 0),
  );
}
 ...
 */
export function generateMockOverrideActivity(
  _storeId: string,
): OverrideActivity {
  return {
 ...
 */
export function generateMockPublishedAudits(
  _storeId: string,
): PublishedAudit[] {
  const shelves = generateMockShelves();
  ...
  const approvedShelves = shelves
    .filter((shelf) => shelf.status === "approved")
    .slice(0, 5);

  approvedShelves.forEach((shelf, index) => {
    const statuses: PublishingStatus[] = [
      "published",
      "published",
      "published",
      "failed",
    ...
    published.push({
      ...
      errorMessage:
        status === "failed" ? "Event bus connection timeout" : undefined,
    });
  });
