...
export function listShelves(fixtureId?: string): Promise<ShelfResponse[]> {
  return axiosClient
    .get<ShelfResponse[]>("/shelves", {
      params: fixtureId ? { fixture_id: fixtureId } : undefined,
    })
    .then((res) => res.data);
}
...
export function getShelf(shelfId: string): Promise<ShelfResponse> {
  return axiosClient
    .get<ShelfResponse>(`/shelves/${shelfId}`)
    .then((res) => res.data);
}
...
export function createShelf(
  ...
  return axiosClient
    .post<ShelfResponse>("/shelves", payload)
    .then((res) => res.data);
}
...
export function updateShelf(
  ...
  return axiosClient
    .put<ShelfResponse>(`/shelves/${shelfId}`, payload)
    .then((res) => res.data);
}
...
export function mapShelfResponseToShelf(res: ShelfResponse): Shelf {
  ...
  const dimensionUnit =
    res.fixture?.dimension_unit ?? res.fixture_dimension_unit;
  const aisleCode =
    res.fixture?.physical_location.aisle ?? res.fixture_aisle ?? "";
