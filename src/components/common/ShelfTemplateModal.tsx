...
import type { ShelfTemplateFixtureType } from "@/types/shelfTemplate";

const PRESET_FIXTURE_OPTIONS: Array<{
  value: ShelfTemplateFixtureType;
  label: string;
}> = [
...
const EXTRA_LABELS_EQUIVALENT_TO_PRESET = new Set(
  ["Gondola (standard)", "Cooler/Chiller", "Wall Unit"].map((s) =>
    fixtureLabelDedupeKey(s),
  ),
);
...
export function buildShelfTemplateFixtureSelectOptions(
  ...
  for (const raw of extraFixtureLabels ?? []) {
    ...
    if (PRESET_FIXTURE_OPTIONS.some((p) => p.value.toLowerCase() === lower))
      continue;
...
export function ShelfTemplateModal({
  ...
  const resolveDepthForFixtureType = (
    fixtureTypeValue: string,
  ): string | undefined => {
    const normalizedValue = fixtureTypeValue.trim().toLowerCase();
    const dedupeValue = fixtureLabelDedupeKey(fixtureTypeValue);
    const optionLabel =
      fixtureOptions.find((option) => option.value === fixtureTypeValue)
        ?.label ?? "";
  ...
  const resolveUnitForFixtureType = (
    fixtureTypeValue: string,
  ): string | undefined => {
    const normalizedValue = fixtureTypeValue.trim().toLowerCase();
    const dedupeValue = fixtureLabelDedupeKey(fixtureTypeValue);
        ...
        <div className="border-border bg-muted/20 flex items-center justify-between border-b p-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-xs">
              Five standard fixture types are always listed. Your store’s custom
              fixture names (from onboarding or Store Defaults) are added
              below—names that duplicate a standard type are hidden so the list
              stays short.
            </p>
          </div>
        </div>
              ...
              <Select
                ...
                onChange={(e) => {
                  ...
                  const depthFromFixture =
                    resolveDepthForFixtureType(selectedFixtureType);
                  setForm((f) => ({
                    ...f,
