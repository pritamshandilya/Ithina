...
export function StoresPage() {
  ...
  const filteredStores = useMemo(() => {
    return (stores as StoreSetting[]).filter(
      (store) =>
  ...
  const columns: DataTableColumn<StoreSetting>[] = [
    ...
    },
    {
      ...
      formatter: (cell: DataTableCell<StoreSetting>) => {
        const value =
          (cell.getValue() as "Active" | "Inactive" | undefined) || "Active";
        const statusClass =
          value === "Inactive"
    ...
    },
    {
      ...
      width: 140,
      formatter: (cell: DataTableCell<StoreSetting>) => {
        const dimensionsIcon = renderToStaticMarkup(
          <RulerDimensionLine size={12} className="opacity-70" />,
        );
        return `
                    <div class="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ${dimensionsIcon}
        ...
        <div className="mx-auto flex min-h-0 w-full max-w-screen-2xl flex-1 flex-col space-y-4">
          <div className="group relative">
            ...
            <Input
              ...
              className="bg-card border-border hover:border-accent/50 focus:border-accent text-foreground placeholder:text-muted-foreground h-12 pl-11 transition-all"
            />
          </div>
