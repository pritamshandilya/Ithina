...
interface UseStoreFixturesTableDomParams {
  ...
  setFixtureComplianceOverrides: Dispatch<
    SetStateAction<Record<string, string>>
  >;
  setFixtureCategorizeOverrides: Dispatch<
    SetStateAction<Record<string, string>>
  >;
...
export function useStoreFixturesTableDom({
  ...
  useEffect(() => {
    ...
    const handleChange = (e: Event) => {
      ...
      if (field === "compliance") {
        setFixtureComplianceOverrides((prev) => ({
          ...prev,
          [fixtureId]: value,
        }));
      }
      if (field === "categorize") {
        setFixtureCategorizeOverrides((prev) => ({
          ...prev,
          [fixtureId]: value,
        }));
      }
  ...
  useEffect(() => {
    ...
    return () =>
      el.removeEventListener("click", handleFixtureHeaderClick, true);
