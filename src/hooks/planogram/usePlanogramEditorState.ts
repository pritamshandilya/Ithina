...
import { applyArrangementToFixtureShelves } from "@/lib/planogram/planogramArrangement";
...
export function usePlanogramEditorState({
  ...
  const baseShelves = useMemo(
    () =>
      localShelves.length > 0
        ? localShelves
        : normalizePlanogramShelves(fixtureShelves),
  ...
  const onRemoveProduct = useCallback(
    (shelfId: string, productId: string) => {
      ...
      setLocalShelves((previous) => {
        ...
        return next.map((shelf) => {
          ...
          const insertionIndex = targetProductId
            ...
            : -1;
          if (insertionIndex >= 0)
            nextProducts.splice(insertionIndex, 0, sourceProduct);
