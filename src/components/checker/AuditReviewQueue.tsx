...
import {
  ...
  type ViewMode,
  filterOptions,
} from "./auditReviewQueue.constants";
import { useAuditReviewQueueData } from "./useAuditReviewQueueData";
import { AuditQueueCard } from "@/components/checker/AuditQueueCard";
import {
  DataTable,
  type DataTableCell,
...
export function AuditReviewQueue({
  ...
  const cardTotalPages = Math.max(
    1,
    Math.ceil(filteredAndSortedAudits.length / CARD_PAGE_SIZE),
  );
  ...
  const visibleCount =
    viewMode === "table" ? tableVisibleCount : paginatedCardAudits.length;
            ...
            <div>
              ...
              {searchQuery.trim() && (
                <button
                  ...
                  type="button"
                  className="text-accent hover:text-accent/80 mt-3 text-sm underline"
                >
                  Clear search
                </button>
              )}
          ...
          <>
            ...
            <div className="mt-3 flex items-center justify-end gap-2">
              <button
                ...
                onClick={() =>
                  setCardPage((page) =>
                    Math.max(1, Math.min(page, cardTotalPages) - 1),
                  )
                }
              ...
              <button
                ...
                onClick={() =>
                  setCardPage((page) =>
                    Math.min(
                      cardTotalPages,
                      Math.min(page, cardTotalPages) + 1,
                    ),
                  )
                }
