  ...
  return {
    ...
    name:
      ("name" in response && typeof response.name === "string"
        ...
        : null) ?? payload.name,
    version:
      ("version" in response ? response.version : undefined) ?? payload.version,
    description:
...
export async function fetchPlanogramList(
  status?: PlanogramApiStatus,
): Promise<PlanogramSummary[]> {
  const response = await axiosClient
    .get<PlanogramResponse[]>("/planograms", {
...
export async function fetchPlanogramById(
  id: string,
): Promise<PlanogramPayload | null> {
  if (!isUuid(id)) {
    return null;
  }
  ...
  try {
    const response = await axiosClient
      .get<PlanogramResponse>(`/planograms/${id}`)
      .then((res) => res.data);
...
export function getAssignPlanogramOverlays(): Map<
  string,
...
): Promise<PlanogramPayload> {
  const response = await axiosClient
    .post<PlanogramResponse>("/planograms", payload as CreatePlanogramPayload)
    .then((res) => res.data);
  return toPlanogramPayload(response);
}
...
export async function deletePlanogram(id: string): Promise<boolean> {
  ...
  try {
    await axiosClient.delete<void>(`/planograms/${id}`);
    return true;
  } catch (error) {
    if (error instanceof ApiError && error.status === 404) {
      return false;
