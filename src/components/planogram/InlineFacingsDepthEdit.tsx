...
export function InlineFacingsDepthEdit({
    ...
    return (
      <div className={cn("flex items-center gap-1", className)}>
        ...
        <input
          ...
          onBlur={commit}
          className="border-input bg-background focus:ring-ring w-8 rounded border px-1 py-0.5 font-mono text-[10px] focus:ring-2 focus:outline-none"
          aria-label="Facings"
        />
        <span className="text-muted-foreground font-mono text-[10px]">D</span>
        <input
          type="number"
          min={1}
          value={editDepth}
  ...
  return (
    <button
      ...
      className={cn(
        "text-muted-foreground hover:bg-muted/50 focus:ring-ring mt-0.5 cursor-text rounded px-1 py-0.5 text-left font-mono text-[10px] leading-tight focus:ring-2 focus:ring-offset-1 focus:outline-none",
        className,
      )}
