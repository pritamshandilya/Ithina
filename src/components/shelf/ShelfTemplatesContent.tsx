...
import { ShelfTemplateModal } from "@/components/common/ShelfTemplateModal";
...
export function ShelfTemplatesContent({
  showHeaderCard = true,
}: ShelfTemplatesContentProps) {
    ...
    try {
      if (mode === "create") {
        ...
        toast({
          ...
          description: "Shelf template saved.",
        });
      } else if (editing) {
    ...
    <Card noBorder className="bg-card/70 shadow-none">
      ...
      <CardContent className="space-y-3">
        {isLoading ? (
          ...
          <div className="border-border bg-muted/20 rounded-lg border border-dashed px-6 py-8 text-center">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {sortedTemplates.map((tpl) => (
              <div
                key={tpl.id}
                className="border-border bg-background/40 space-y-2 rounded-xl border p-4"
              >
