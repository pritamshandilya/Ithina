...
import type { AuditQueueFilter } from "@/types/checkerUi";
...
export const filterOptions: {
  ...
  { value: "all", label: "All Pending", count: (audits) => audits.length },
  {
    ...
    label: "Critical",
    count: (audits) =>
      audits.filter((a) => (a.complianceScore || 0) < 50).length,
  },
  {
    value: "attention",
  ...
  },
  {
    ...
    label: "Good",
    count: (audits) =>
      audits.filter((a) => (a.complianceScore || 0) >= 80).length,
  },
  {
    value: "planogram",
    ...
    count: (audits) =>
      audits.filter(
        (a) => a.mode === "planogram-based" || a.mode === "vision-edge",
      ).length,
  },
  {
    value: "adhoc",
    ...
    count: (audits) =>
      audits.filter((a) => a.mode === "adhoc" || a.mode === "assist-mode")
        .length,
  },
];
...
export const AUDIT_BASE_TABLE_COLUMNS: DataTableColumn<CheckerAudit>[] = [
  ...
  },
  {
    ...
    formatter: (cell) => {
      const value = (
        cell as { getValue: () => string | Date | undefined }
      ).getValue();
