...
const RULE_CATEGORIES: RuleType[] = [
  "VISUAL",
  "SAFETY",
  "PROFITABILITY",
  "EFFICIENCY",
...
export function RuleBuilderModal({
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (rule) {
        const rulesToLoad =
          rulesInSet && rulesInSet.length > 0 ? rulesInSet : [rule];
  ...
  const syncDocumentLinks = useCallback(
    async (targetRuleIds: string[]) => {
      ...
      for (const doc of documents) {
        ...
        const hasAnyTargetLink = targetRuleIds.some((id) =>
          doc.linkedRuleIds.includes(id),
        );
  ...
  const validateEdit = (): boolean => {
    ...
    if (enabledRules.length === 0)
      next.rules = "At least one rule must be enabled.";
    rules.forEach((r) => {
      if (r.enabled && !r.name.trim())
        next[`rule-${r.id}-name`] = "Rule name is required.";
    });
    setErrors(next);
    return Object.keys(next).length === 0;
  };
  ...
  const saveAsDraft = useCallback(
    async (lenient: boolean): Promise<boolean> => {
      ...
      if (lenient && rulesToSave.length === 0 && rules.length > 0) {
        rulesToSave = [
          { ...rules[0]!, name: rules[0]!.name.trim() || "Untitled" },
        ];
      }
  ...
  const handleSubmitEdit = async () => {
    ...
    const existingRuleIds = new Set(
      (rulesInSet ?? [rule]).map((r) => r.ruleId),
    );
    let successCount = 0;
    let lastError: Error | null = null;
  ...
  const handleClose = async () => {
    ...
    const hasContent =
      ruleSetName.trim() ||
      rules.some(
        (r) => r.name.trim() || r.description.trim() || r.threshold.trim(),
      );
    if (hasContent) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} className="max-w-4xl">
      <div className="border-border bg-card flex max-h-[90vh] flex-col rounded-lg border shadow-lg">
        {/* Header */}
        <div className="border-border flex shrink-0 items-center justify-between border-b px-5 pt-5 pb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleClose}
              className="text-muted-foreground hover:bg-muted hover:text-foreground focus:ring-ring rounded-md p-2 transition-colors focus:ring-2 focus:outline-none"
              aria-label="Close"
            >
              <X className="size-4" />
            </button>
        ...
        <div className="flex min-h-0 flex-1 flex-col px-5 py-3">
          <div className="flex min-h-0 flex-1 flex-col gap-4">
            ...
            <div className="border-border bg-card/40 shrink-0 rounded-lg border p-3">
              ...
              <div className="border-border/80 mt-2 max-h-28 overflow-y-auto rounded-md border">
                {filteredDocuments.length === 0 ? (
                  ...
                  filteredDocuments.map((doc) => (
                    <label
                      key={doc.id}
                      className="border-border/70 hover:bg-muted/40 flex cursor-pointer items-center justify-between gap-2 border-b px-2 py-1.5 text-sm last:border-b-0"
                    >
            ...
            <div className="border-border min-h-0 flex-1 overflow-hidden overflow-y-auto rounded-lg border">
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-muted/50 border-border border-b">
                    <th className="w-10 p-2 text-left" scope="col">
                      <span className="sr-only">Enabled</span>
                    </th>
                    <th
                      className="text-foreground p-2 text-left font-medium"
                      scope="col"
                    >
                      Rule name
                    </th>
                    ...
                    >
                      ...
                      <td className="p-2 align-top">
                        <Input
                          ...
                          value={r.name}
                          onChange={(e) =>
                            updateRuleItem(r.id, { name: e.target.value })
                          }
                          className="h-8 w-full min-w-[120px] font-medium"
                          disabled={isRetired}
                        />
                      </td>
                      <td className="p-2 align-top">
                        <Input
                          ...
                          className="text-muted-foreground h-8 w-full min-w-[140px] text-sm"
                          disabled={isRetired}
                        />
                      </td>
                      <td className="p-2 align-top">
                      ...
                      <td className="p-2 align-top">
                        {!isRetired && rules.length > 1 ? (
                          <button
                            ...
                            className="text-muted-foreground hover:bg-destructive/20 hover:text-destructive rounded p-2 transition-colors"
            ...
            {isRetired && (
              <p className="border-border bg-muted/30 text-muted-foreground shrink-0 rounded-lg border p-3 text-sm">
                Retired rules cannot be edited. Clone the rule to create a new
                version.
              </p>
            )}
          </div>
