 ...
 * Data source is provided by report container.
 */
...
import type {
  ...
  PlanogramItemRow,
  SkuFacingRow,
} from "@/lib/analysis/allItemsReportTypes";
...
export interface AllItemsTabProps {
  ...
  /** Hide expected-planogram section for adhoc analyses */
  showPlanogramItems?: boolean;
...
export function AllItemsTab({
  ...
  const planogramItems = data?.planogramItems ?? [];
  const skuFacings = data?.skuFacings ?? [];

  const filteredPlanogramItems = useMemo(
    () =>
      filterRows(planogramItems, searchQuery, [
        "productName",
        "sku",
        "shelf",
        "issueDescription",
      ]),
    [planogramItems, searchQuery],
  );

  const filteredSkuFacings = useMemo(
    () => filterRows(skuFacings, searchQuery, ["productName", "sku"]),
    [skuFacings, searchQuery],
  );
    ...
    <div className={cn("w-full min-w-0 space-y-4", className)}>
      ...
      <section className="min-w-0">
        <h3 className="text-foreground mb-3 text-sm font-semibold tracking-wider uppercase">
          SKU Facings & Depth Summary ({filteredSkuFacings.length})
        </h3>
