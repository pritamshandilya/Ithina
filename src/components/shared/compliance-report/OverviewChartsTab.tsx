...
export function OverviewChartsTab({ report, className }: OverviewChartsTabProps) {
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      {/* Space Efficiency vs Weight (placeholder chart) */}
      <ReportCard title="Space Efficiency vs Weight" icon={Leaf}>
        <div className="relative h-[200px] rounded-lg border border-border bg-muted/20 flex items-center justify-center">
          <p className="text-sm text-muted-foreground">
            Placeholder: Scatter/bubble chart (Weight vs Revenue/sq ft)
          </p>
          <div className="absolute bottom-2 right-2 rounded border border-border bg-card/80 px-2 py-1.5 text-[10px] text-muted-foreground max-w-[140px]">
            <strong className="text-foreground">How to read:</strong> X = weight
            (kg), Y = revenue/sq ft, bubble = unit margin. Green = high
            space-efficiency, Indigo = lower contribution/sq ft.
          </div>
        </div>
      </ReportCard>
