...
export type UpdateComplianceRuleSetInput = Partial<
  Omit<CreateComplianceRuleSetInput, "rules">
> & {
...
export function formatDefaultOnboardingComplianceRuleSetName(
  storeName: string,
): string {
...
function toRuleStatusFrom(
  ruleSetStatus: ComplianceRuleSetStatus,
  isActive: boolean,
 ...
 */
export async function fetchComplianceRuleSetsForAnalysis(): Promise<
  ComplianceRuleSetSummary[]
> {
 ...
 */
export async function fetchAggregatedComplianceRulesFromRuleSets(): Promise<
  ComplianceRule[]
> {
  const sets = await axiosClient
    .get<ComplianceRuleSetRequestResponse[]>(COMPLIANCE_RULE_SETS_ENDPOINT)
...
export async function fetchRulesByRuleSetId(
  ruleSetId: string,
