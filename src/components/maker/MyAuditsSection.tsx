...
export function MyAuditsSection({
  ...
  const { data: draftAudits = [], isLoading: isDraftsLoading } =
    useDraftAudits();
  const { data: returnedAudits = [], isLoading: isReturnedLoading } =
    useReturnedAudits();
  ...
  const handleAction = useCallback(
    (audit: Audit, action: "resume" | "fix" | "delete") => {
      if (action === "delete") {
  ...
  return (
    <div className={cn("space-y-4", className)}>
        ...
        <div className="scrollbar-thin flex gap-2 overflow-x-auto pb-2">
          {(["all", "draft", "returned"] as const).map((filter) => (
            <button
              ...
              className={cn(
                "inline-flex shrink-0 items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium transition-all",
      ...
      <div className="border-border bg-card min-h-0 flex-1 overflow-hidden rounded-lg border">
        <DataTable<Audit>
