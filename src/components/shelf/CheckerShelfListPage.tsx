...
import { CheckerShelfEmptyState } from "./CheckerShelfEmptyState";
import { CheckerShelfListToolbar } from "./CheckerShelfListToolbar";
import {
...
function parseBulkShelvesPayload(raw: string): ParsedBulkPayload {
  ...
  const normalizedFixtures: BulkFixtureInput[] = fixtures.map(
    (fixture, fixtureIdx) => {
      ...
      if (!code)
        throw new Error(`Fixture ${fixtureIdx + 1}: \`code\` is required.`);
      if (!type)
        throw new Error(`Fixture ${fixtureIdx + 1}: \`type\` is required.`);
      if (!dimensionUnit)
        throw new Error(
          `Fixture ${fixtureIdx + 1}: \`dimension_unit\` is required.`,
        );
      if (!dimensions || typeof dimensions !== "object") {
        throw new Error(
          `Fixture ${fixtureIdx + 1}: \`dimensions\` is required.`,
        );
      }
      if (!location || typeof location !== "object") {
        throw new Error(
          `Fixture ${fixtureIdx + 1}: \`physical_location\` is required.`,
        );
      }
      if (!Array.isArray(shelves) || shelves.length === 0) {
        throw new Error(
          `Fixture ${fixtureIdx + 1}: \`shelves\` must be a non-empty array.`,
        );
      }
      ...
      const normalizedShelves = shelves.map((shelf, shelfIdx) => {
        ...
        if (!shelfCode) {
          throw new Error(
            `Fixture ${fixtureIdx + 1}, shelf ${shelfIdx + 1}: \`code\` is required.`,
          );
        }
        if (!shelfName) {
          throw new Error(
            `Fixture ${fixtureIdx + 1}, shelf ${shelfIdx + 1}: \`name\` is required.`,
          );
        }
...
export function CheckerShelfListPage({
  ...
  const storeId =
    getOptionalStoreId(params) ?? (isAdminPath ? selectedStore?.id : undefined);
  ...
  const [shelfIdPendingDelete, setShelfIdPendingDelete] = useState<
    string | null
  >(null);
    ...
    return base.map((col) => {
      ...
      if (field === "width" || field === "height" || field === "depth") {
        return {
          ...
          width:
            typeof col.width === "number" ? Math.max(col.width, 88) : col.width,
          hozAlign: "right",
      ...
      if (col.title === "Action") {
        return {
          ...
          width:
            typeof col.width === "number" ? Math.max(col.width, 64) : col.width,
        };
      }
      return col;
    });
          ...
          <div className="shrink-0 space-y-3">
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm">
                Upload or paste JSON, then review the parsed shelves before
                creating.
              </p>
            </div>
