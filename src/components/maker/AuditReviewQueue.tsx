      ...
      },
      {
        ...
        cellClick: (event: unknown, cell: { getData: () => Audit }) => {
          ...
          if (audit.status === "returned" || audit.status === "draft") {
            onAction?.(
              audit.id,
              audit.status === "returned" ? "fix" : "resume",
            );
          }
        },
      },
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap gap-2 pb-2">
          {(["all", "draft", "returned", "pending", "approved"] as const).map(
            (filter) => (
              <button
                key={filter}
                type="button"
                ...
                className={cn(
                  "inline-flex shrink-0 items-center gap-2 rounded-lg border px-3 py-1.5 text-sm font-medium transition-all",
      ...
      {filteredAudits.length === 0 ? (
        <div className="border-border bg-card rounded-lg border p-12 text-center shadow-sm">
