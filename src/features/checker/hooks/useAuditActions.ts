...
import { approveAudit, returnAudit, overrideAndApprove } from "../api/checker";
...
export function useApproveAudit(storeId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: pendingAuditsKeys.byStore(storeId) });
      queryClient.invalidateQueries({ queryKey: complianceOverviewKeys.byStore(storeId) });
      queryClient.invalidateQueries({ queryKey: publishedAuditsKeys.byStore(storeId) });
    },
  });
}
...
export function useOverrideAndApprove(storeId: string) {
  ...
  return useMutation({
    mutationFn: ({ auditId, overrideReason }: { auditId: string; overrideReason: string }) =>
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: complianceOverviewKeys.byStore(storeId) });
      queryClient.invalidateQueries({ queryKey: publishedAuditsKeys.byStore(storeId) });
      queryClient.invalidateQueries({ queryKey: overrideActivityKeys.byStore(storeId) });
    },
  });
}
