...
export function OverviewSpaceEfficiencyChart() {
  ...
  const highEfficiencyData = chartData.filter((point) => point.isHighEfficiency);
  const lowEfficiencyData = chartData.filter((point) => !point.isHighEfficiency);
    ...
    <div className="relative w-full">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <ScatterChart margin={{ top: 14, right: 18, bottom: 26, left: 14 }}>
          <CartesianGrid stroke="var(--border)" strokeDasharray="4 4" strokeOpacity={0.7} />
          <XAxis
            type="number"
            dataKey="weightKg"
            domain={[X_MIN, X_MAX]}
          ...
          <YAxis
            ...
            tickLine={false}
          />
          <ZAxis type="number" dataKey="bubbleSize" range={[BUBBLE_MIN_SIZE, BUBBLE_MAX_SIZE]} />
          <Tooltip content={<SpaceEfficiencyTooltip />} cursor={{ stroke: "var(--border)" }} />
          <Scatter
      ...
      <div className="absolute top-1 right-1 z-10">
        <div className="group relative">
          ...
          <div className="border-border bg-card/95 text-muted-foreground pointer-events-none absolute top-8 right-0 hidden w-[170px] rounded-md border px-2.5 py-2 text-[10px] shadow-lg group-hover:block">
            ...
            <p className="text-chart-1 mt-0.5">Purple: lower efficiency items</p>
