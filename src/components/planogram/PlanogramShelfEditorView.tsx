...
import { useStoreFixtureDetailPlanogramEditor } from "@/components/checker/stores/useStoreFixtureDetailPlanogramEditor";
...
import { sortPlanogramShelves } from "@/lib/planogram/planogramSchema";
      ...
      <div className="bg-primary min-h-screen px-2 pt-2 pb-4 sm:px-2 sm:pt-3 sm:pb-4 lg:px-2 lg:pt-4 lg:pb-5">
        <div className="mx-auto max-w-screen-2xl space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="border-border bg-card/80 text-destructive rounded-lg border p-6 text-sm">
              {error instanceof Error
                ? error.message
                : "Failed to load planogram."}
            </div>
          ) : !payload ? (
            <div className="border-border bg-card/80 text-muted-foreground rounded-lg border p-6 text-sm">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-6">
                <StatCard
                  title="Shelves"
                  value={editor.stats.shelves}
                  className="stat-card"
                />
                <StatCard
                  title="SKUs"
                  value={editor.stats.skus}
                  className="stat-card"
                />
                <StatCard
                  title="Front Facings"
                  value={editor.stats.frontFacings}
                  className="stat-card"
                />
                <StatCard
                  title="Total Units"
                  value={editor.stats.totalUnits}
                  className="stat-card"
                />
                <StatCard
                  title="Categories"
                  value={editor.stats.categories}
                  className="stat-card"
                />
                <StatCard
                  title="Removed"
                  value={editor.stats.removed}
                  className="stat-card"
                />
              </div>
                    ...
                    <ShelfRow
                      ...
                      dragHandlers={{
                        ...
                        onDropOnShelf: (toShelfId, targetProductId) => {
                          ...
                          const { productId, fromShelf } =
                            editor.dragRef.current;
                        ...
                        onDropOnRemoved: () => {
                          if (
                            !editor.dragRef.current ||
                            editor.dragRef.current.fromShelf === "removed"
                          ) {
                            return;
                          }
                          const { productId, fromShelf } =
                            editor.dragRef.current;
                          editor.dragRef.current = null;
                          editor.editHandlers.onRemoveProduct(
                            fromShelf,
                            productId,
                          );
                        },
              ...
              <div className="grid gap-3">
                ...
                <div className="border-border bg-card/80 text-muted-foreground rounded-lg border p-3 text-sm">
                  {payload.status} · Fixture {payload.fixture.width}×
                  {payload.fixture.height}×{payload.fixture.depth}
                </div>
              </div>
            </div>
          )}
