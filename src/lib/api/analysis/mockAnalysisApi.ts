...
import { generateMockAdhocAnalyses } from "@/lib/api/mockData";
import {
  PLANOGRAM_POC_001,
  PLANOGRAM_POC_002,
} from "@/lib/api/planogramSample";
...
export const mockAnalysisApiClient: AnalysisApiClient = {
  ...
  async fetchPlanogramById(id: string): Promise<PlanogramPayload | null> {
    ...
    return (
      allPlanogramPayloads().find((record) => record.id === id)?.payload ?? null
    );
  },
  async savePlanogramPayload(
    payload: PlanogramPayload,
