...
/* ─── Static data matching the HTML prototype ─── */

const STAT_CARDS = [
  { label: "Active Campaigns", value: "12", trend: "+2 This Week", trendClass: "text-emerald-400" },
  { label: "Pending Approvals", value: "3", trend: "Requires Action", trendClass: "text-amber-500" },
  { label: "Rejected Campaigns", value: "1", trend: "Needs Revision", trendClass: "text-rose-400" },
];
...
const ROUTE_BY_STATUS: Record<HistoryStatus, string> = {
  live:    "/maker/fleet",
  ...
  return (
    <div className="flex w-full flex-1 flex-col gap-6 overflow-y-auto p-6 lg:p-8 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="grid shrink-0 grid-cols-1 gap-6 md:grid-cols-3">
        {STAT_CARDS.map((card) => (
          <div key={card.label} className="rounded-xl border border-ithina-border bg-ithina-panel p-5 shadow-sm">
            <p className="mb-1 font-mono text-[10px] uppercase tracking-widest text-slate-500">{card.label}</p>
      ...
      <div className="flex min-h-[300px] flex-1 flex-col overflow-hidden rounded-2xl border border-ithina-border bg-ithina-panel">
        ...
        <IthTable<CampaignRow>
          ...
          rowKey={(r) => r.id}
          onRowClick={(row) => navigate({ to: ROUTE_BY_STATUS[row.status] as never })}
          rowHighlight={(row) => {
