...
import { useMakerAudits, useShelves } from "@/queries/maker";
...
function getModeLabel(mode: string): string {
  if (mode === "planogram-based" || mode === "vision-edge")
...
export function ApprovalStatusList({
  ...
  const submittedAudits = useMemo(() => {
    return (allAudits || []).filter(
      (a) => a.status !== "draft" && a.status !== "never-audited",
    );
  }, [allAudits]);

  const normalizedAudits = useMemo(() => {
    return submittedAudits.map((audit) => {
      const submittedAt = audit.submittedAt
        ? new Date(audit.submittedAt)
        : undefined;
      let approvedAt = audit.approvedAt
    ...
    () => [
      ...
      },
      {
        ...
        cellClick: (event: unknown, cell: { getData: () => Audit }) => {
          ...
          const action = (btn as HTMLElement).getAttribute(
            "data-action",
  ...
  return (
    <div className={cn("flex min-h-0 flex-1 flex-col", className)}>
      ...
      <div className="mt-2 flex shrink-0 flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap gap-2">
          {(["all", "pending", "approved", "returned"] as const).map(
            (filter) => (
              <button
                key={filter}
                type="button"
      ...
      <div className="mt-2 flex min-h-0 flex-1 flex-col overflow-hidden">
        {filteredAudits.length === 0 ? (
          <div className="border-border bg-card flex min-h-full items-center justify-center rounded-lg border p-8 text-center">
            <div>
              <div className="bg-muted mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full">
