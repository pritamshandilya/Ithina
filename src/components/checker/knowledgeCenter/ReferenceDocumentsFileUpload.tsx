...
import {
  ...
  getFileExtension,
  getFileTypeTone,
} from "./referenceDocumentsTab.utils";
  ...
  const validateAndSelectFiles = useCallback(
    (files: File[]) => {
      ...
      const existing = new Set(
        uploadedFiles.map((file) => `${file.name}-${file.size}`),
      );
      const valid: File[] = [];
      ...
      const totalSizeBytes = nextFiles.reduce(
  ...
  if (variant === "minimal") {
    return (
      <div className={className}>
        ...
        <label
          htmlFor={inputId}
          className="text-accent hover:text-accent/80 inline-flex cursor-pointer items-center gap-1.5 text-xs font-medium"
        >
    ...
    return (
      <div className={className}>
        ...
        >
          ...
          <label
            htmlFor={inputId}
            className="text-muted-foreground flex cursor-pointer items-center justify-center gap-2 text-xs font-medium"
          >
    ...
    <div className={className}>
      ...
      >
        ...
        {!error && (
          <label
            htmlFor={inputId}
            className="border-border bg-background hover:bg-muted mt-4 inline-flex cursor-pointer items-center gap-1 rounded-md border px-3 py-1.5 text-xs font-medium"
          >
            <Upload className="size-3.5" />
