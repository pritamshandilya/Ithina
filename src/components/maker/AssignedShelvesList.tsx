...
export function AssignedShelvesList({
  ...
  const cardTotalPages = Math.max(
    1,
    Math.ceil(filteredShelves.length / CARD_PAGE_SIZE),
  );
    ...
    return (
      <div className={cn("space-y-4", className)}>
        <div className="bg-card border-border rounded-lg border p-12 text-center">
          ...
          <p className="text-muted-foreground mt-2 text-sm">
            You don't have any shelves assigned yet. Contact your manager to get
            started.
          </p>
        </div>
      </div>
        ...
        {filterOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={cn(
                "shrink-0 rounded-lg px-4 py-2 text-sm font-medium whitespace-nowrap transition-all",
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              onClick={() =>
                setCardPage((page) =>
                  Math.min(cardTotalPages, Math.min(page, cardTotalPages) + 1),
                )
              }
