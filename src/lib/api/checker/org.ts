...
function mapOrgStore(store: OrgStoreApiModel): Store {
  ...
  return {
    ...
    default_compliance_rule_set_id:
      store.default_compliance_rule_set_id ?? null,
    is_active: isActive,
...
export async function fetchOrganization(): Promise<OrganizationInfo> {
  const response = await axiosClient.get<OrganizationInfo>("/organization");
  return response.data;
}

export async function fetchOrgStores(): Promise<Store[]> {
  const response = await axiosClient.get<OrgStoreApiModel[]>("/stores");
  const stores = response.data;
  return stores.map(mapOrgStore);
}

export async function fetchScopedStore(storeId: string): Promise<Store> {
  const response = await axiosClient.get<OrgStoreApiModel>("/store", {
...
export async function fetchStoreById(storeId: string): Promise<Store> {
  const response = await axiosClient.get<OrgStoreApiModel>(
    `/stores/${storeId}`,
  );
  const store = response.data;
  return mapOrgStore(store);
...
export async function assignUserToStore(
  ...
  await axiosClient.put<void>(`/store/users/${userId}`, undefined, {
    headers: { "X-Store-Id": storeId },
  });
}
...
export async function removeUserFromStore(
  ...
  await axiosClient.delete<void>(`/store/users/${userId}`, {
    headers: { "X-Store-Id": storeId },
  });
}
...
export async function fetchOrgUsers(
  ...
  const response = await axiosClient.get<OrgUserApiModel[]>(
    "/organization/users",
    { params },
  );
...
export async function fetchStoreUsers(
  ...
  const response = await axiosClient.get<OrgUserApiModel[]>("/store/users", {
    params,
    headers: { "X-Store-Id": storeId },
  });
  const users = response.data;
...
export async function createStore(data: {
  ...
  const response = await axiosClient.post<OrgStoreApiModel>("/stores", payload);
  const store = response.data;
  return mapOrgStore(store);
}

export async function deleteStore(storeId: string): Promise<void> {
  await axiosClient.delete<void>("/store", {
    headers: { "X-Store-Id": storeId },
  });
}
...
export async function updateStore(
  ...
  const response = await axiosClient.put<OrgStoreApiModel>("/store", payload, {
    headers: { "X-Store-Id": storeId },
  });
  const store = response.data;
  return mapOrgStore(store);
...
export async function createUser(
  ...
  const response = await axiosClient.post<OrgUserApiModel>("/users", payload);
...
export async function inviteUser(
  ...
  const response = await axiosClient.post<OrgUserApiModel>(
    "/users/invite",
    payload,
  );
  const user = response.data;
...
export async function fetchUserById(userId: string): Promise<AuthSessionUser> {
  const response = await axiosClient.get<OrgUserApiModel>(`/users/${userId}`);
  const user = response.data;
  return mapOrgUser(user);
}
...
export async function updateUser(
  ...
  const response = await axiosClient.put<OrgUserApiModel>(
    `/users/${userId}`,
    payload,
  );
  const user = response.data;
...
export async function deactivateUser(userId: string): Promise<void> {
  await axiosClient.delete<void>(`/users/${userId}`);
}
