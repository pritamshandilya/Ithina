...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-foreground text-lg font-semibold leading-tight">{value}</p>
    </article>
  );
}
...
export function ReportStatsStrip({ report }: { report: ReportSnippet }) {
  return (
    <section className="grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-8">
      <StatCard label="Compliance" value={`${report.complianceScore}%`} tone="bad" />
...
export function ReportOverview({
  ...
  const maxIssue = Math.max(1, ...report.issueCategories.map((entry) => entry.count));
    ...
    <div className="grid gap-3 xl:grid-cols-3">
          ...
          {report.keyFindings.map((finding, index) => (
            <div
              ...
              className={cn(
                ...
                finding.type === "error" && "border-destructive/40 bg-destructive/10",
      ...
      <section className="border-border bg-card/60 rounded-xl border p-4">
        ...
        <ul className="space-y-2">
          {report.aiRecommendations.slice(0, 5).map((item, index) => (
            <li key={`${item}-${index}`} className="text-muted-foreground text-sm">
              - {item}
            </li>
          ))}
        </ul>
          ...
          {report.shelfCompliance.map((row) => (
            <div key={row.shelfName}>
              <div className="mb-1 flex items-center justify-between text-xs">
                ...
                <span className="text-foreground font-medium">{row.compliance}%</span>
              </div>
      ...
      <section className="border-border bg-card/60 rounded-xl border p-4 xl:col-span-2">
        ...
        <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
          <StatCard label="Matched" value={report.issueDistribution.matched} tone="good" />
          <StatCard
            label="Misplaced"
            value={report.issueDistribution.misplaced}
            tone="warn"
          />
          <StatCard label="Missing" value={report.issueDistribution.missing} tone="bad" />
          <StatCard label="Unexpected" value={report.issueDistribution.extra} tone="warn" />
        </div>
        <div className="mt-3 space-y-2">
      ...
      {showOverlay && overlays.length > 0 ? (
        <div className="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-4">
          <StatCard
            ...
            value={overlays.filter((overlay) => overlay.status === "compliant").length}
            tone="good"
          />
          <StatCard
            label="Misplaced markers"
            value={overlays.filter((overlay) => overlay.status === "misplaced").length}
            tone="warn"
          />
          <StatCard
            label="Missing markers"
            value={overlays.filter((overlay) => overlay.status === "missing").length}
            tone="bad"
          />
          <StatCard
            label="Extra markers"
            value={overlays.filter((overlay) => overlay.status === "extra").length}
            tone="warn"
          />
        </div>
      ) : null}
...
export function ReportIssuesPanel({ data }: { data?: AllIssuesReportData | null }) {
  const issues = useMemo(() => {
    ...
    flattened.forEach((issue) => {
      const key = `${issue.group}-${issue.productName}-${issue.description}`.toLowerCase();
      if (!unique.has(key)) {
          ...
          <tbody>
            {issues.map((issue) => (
              <tr key={issue.id} className="border-t">
                <td className="p-2 text-muted-foreground">{issue.group}</td>
                <td className="p-2">{issue.productName}</td>
                <td className="p-2 text-muted-foreground">{issue.description}</td>
                <td className="p-2 text-muted-foreground">{issue.detail ?? "-"}</td>
                <td className="p-2">{issue.severity}</td>
              </tr>
            ))}
            {issues.length === 0 && (
              <tr>
                <td className="text-muted-foreground p-4 text-center" colSpan={5}>
                  No issues detected.
                </td>
              </tr>
            )}
...
export function ReportItemsPanel({ data }: { data?: AllItemsReportData | null }) {
  const items = useMemo(() => {
    ...
    return rows
      ...
      .sort((a, b) => {
        ...
        const rankDiff = severityRank[a.facingDiffVariant] - severityRank[b.facingDiffVariant];
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-t">
                ...
                <td className="p-2 text-muted-foreground">{item.frontFacings}</td>
                <td className="p-2 text-muted-foreground">{item.detected}</td>
                <td className="p-2 text-muted-foreground">{item.facingDiffText}</td>
